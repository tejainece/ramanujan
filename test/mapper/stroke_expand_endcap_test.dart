...
void main() {
  ...
  group('open-end cap', () {
    test('non-zero widthAtP2 emits a cap that gives the end its width', () {
      const w2 = 80.0;
      final out = strokeExpand([cubic], maxWidth: 80, widthAtP1: 0, widthAtP2: w2);
      final caps = out.whereType<LineSegment>().toList();
      expect(caps, isNotEmpty, reason: 'expected a cap segment at the wide end');
      // The cap spans the full end width.
      final cap = caps.first;
      expect((cap.p1 - cap.p2).length, closeTo(w2, 1e-6));
    });

    test('the two end edges meet the cap (outline is connected at the end)', () {
      final out = strokeExpand([cubic], maxWidth: 80, widthAtP1: 0, widthAtP2: 80);
      // Walk the outline; every consecutive pair must be continuous.
      for (var i = 0; i < out.length - 1; i++) {
        expect(out[i].p2.isEqual(out[i + 1].p1), isTrue,
    ...
    test('closed path still gets its seam', () {
      final circle = strokeExpand([
        CircularArcSegment(const P(-120, 0), const P(120, 0), 120, clockwise: false),
        CircularArcSegment(const P(120, 0), const P(-120, 0), 120, clockwise: false),
      ], maxWidth: 40);
