...
void main() {
  group('notcher', () {
    ...
    Notch nick(double t, {double depth = 6, double half = 4, double tilt = 0}) =>
        Notch(t: t, depth: depth, halfBefore: half, halfAfter: half, tilt: tilt);
    ...
    test('keeps the original endpoints', () {
      final pts =
          notcher([nick(0.25), nick(0.5), nick(0.75)])(segment).polylinePoints;
    ...
    test('skips a notch that overlaps the previous one', () {
      ...
      final pts =
          notcher([nick(0.5, half: 30), nick(0.55, half: 30)])(segment)
              .polylinePoints;
      final apexes = pts.where((p) => p.y.abs() > 1e-6).toList();
