...
P _svgArcMidpoint(
  ...
  var factor = den == 0
      ? 0.0
      : math.sqrt((num / den).clamp(0, double.infinity));
...
void main() {
  group('CircularArcSegment radius correction (chord > 2*radius)', () {
    ...
    test('lerp(0.5) matches the rendered midpoint', () {
      final rendered = _svgArcMidpoint(
        degenerate.p1,
        degenerate.p2,
        degenerate.radius,
        largeArc: degenerate.largeArc,
        sweep: !degenerate.clockwise,
      );
    ...
    test('endpoints lie on the (corrected) circle', () {
      expect(
        (degenerate.center - degenerate.p1).length,
        closeTo(degenerate.effectiveRadius, 1e-6),
      );
      expect(
        (degenerate.center - degenerate.p2).length,
        closeTo(degenerate.effectiveRadius, 1e-6),
      );
    });

    test('valid arcs (chord <= 2*radius) are unaffected', () {
      final valid = CircularArcSegment(
        const P(-130, 0),
        const P(300, 0),
        250,
        clockwise: false,
  ...
  group('strokeExpand: rendered arc bulges to the correct side', () {
    final arcs = <String, CircularArcSegment>{
      'small ccw': CircularArcSegment(
        const P(-120, 0),
        const P(120, 0),
        200,
        clockwise: false,
      ),
      'small cw': CircularArcSegment(
        const P(-120, 0),
        const P(120, 0),
        200,
        clockwise: true,
      ),
      'large ccw': CircularArcSegment(
        const P(-120, 0),
        const P(120, 0),
        130,
        largeArc: true,
      ...
      'quarter ccw': CircularArcSegment(
        const P(0, 100),
        const P(100, 0),
        100,
        clockwise: false,
      ),
      'quarter cw': CircularArcSegment(
        const P(0, 100),
        const P(100, 0),
        100,
        clockwise: true,
        ...
        test('$label / $wlabel', () {
          ...
          for (final s in result) {
            ...
            expect(
              ...
              isTrue,
              reason:
                  '$label/$wlabel: rendered midpoint $rendMid is on the '
