...
Loop _circleLoop(P c, double r) => Loop([
      CircularArcSegment(P(c.x + r, c.y), P(c.x, c.y + r), r,
          clockwise: false),
      CircularArcSegment(P(c.x, c.y + r), P(c.x - r, c.y), r,
          clockwise: false),
      CircularArcSegment(P(c.x - r, c.y), P(c.x, c.y - r), r,
          clockwise: false),
      CircularArcSegment(P(c.x, c.y - r), P(c.x + r, c.y), r,
...
void main() {
  ...
  group('PathDivision', () {
    ...
    test('division of overlapping rects splits A into 2 pieces (intersection and difference)', () {
      final result = op.compute(_a, _b);
      expect(result.loops, hasLength(2));
    });
  ...
  group('PathFracture', () {
    ...
    test('fracture of overlapping rects splits into 3 pieces (A-B, B-A, and intersection)', () {
      final result = op.compute(_a, _b);
  ...
  group('Region.separateDisconnected', () {
    test('separateDisconnected of disconnected rects splits into separate regions', () {
      final region = Region([
      ...
      expect(_totalArea(pieces[0]) + _totalArea(pieces[1]), closeTo(20000, 1));
    });
    ...
    test('separateDisconnected of donut with island splits into two regions', () {
      final donut = _annulus(P(0, 0), 100, 50);
